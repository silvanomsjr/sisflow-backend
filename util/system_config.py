...
class SystemConfiguration:
    ...
    def __load_mails(self):
        ...
        if not self.coordinator_email:
            config_mail = ConfigRepository.read_config_mail(
                config_name="coordinator mail"
            )
    ...
    def __load_paths(self):
        ...
        if not self.key_files_path or not self.user_files_path:
            ...
            print("caminhoRoot: ", root_path)

            try:
                self.key_files_path = root_path / Path(
                    ConfigRepository.read_config_system_path(
                        config_name="root path key files"
                    ).system_path
                )
                self.user_files_path = root_path / Path(
                    ConfigRepository.read_config_system_path(
                        config_name="root path user files"
                    ).system_path
                )
                print("testando key: ", self.key_files_path)
                print("testando user: ", self.user_files_path)
            except Exception as e:
                logging.warn(
                    "System error: Error trying to load system file paths data", e
                )
                exit()
    ...
    def __load_holidays(self):
        ...
        if not config_year:
            ...
            holidays_res = None
            try:
                holidays_res = requests.get(
                    "https://brasilapi.com.br/api/feriados/v1/" + actual_year
                ).json()
            except Exception as e:
                logging.warn("Error trying to load holidays from external API", e)
                exit()

            actual_config = ConfigRepository.create_config(
                config_name=f"year {str(actual_year)}"
            )
            actual_config_year = ConfigRepository.create_config_year(
                config_id=actual_config.id, year=actual_year
            )

            for holiday in holidays_res:
                actual_config_year_holiday = (
                    ConfigRepository.create_config_year_holiday(
    ...
    def get_key_files_path(self):
        return (
            self.key_files_path / "private-key.pem",
            self.key_files_path / "public-key.pem",
        )
    ...
    def sistem_str_parser(self, raw_str, student_data=None, advisor_data=None):
        ...
        if student_data:
            student_profile = SystemConfiguration.get_user_token_profile(
                student_data, "STU"
            )
        if advisor_data:
            advisor_profile = SystemConfiguration.get_user_token_profile(
                advisor_data, "ADV"
            )
        ...
        while parser_substr:
            ...
            if "studentName" in command:
                raw_str = raw_str.replace(
                    parser_substr, student_data.get("user_name") if student_data else ""
                )
            elif "advisorName" in command:
                raw_str = raw_str.replace(
                    parser_substr, advisor_data.get("user_name") if advisor_data else ""
                )
            ...
            # replace with matricula
                ...
                raw_str = raw_str.replace(
                    parser_substr,
                    student_profile.get("matricula") if student_profile else "",
                )
            ...
            # replace with student course
                ...
                raw_str = raw_str.replace(
                    parser_substr,
                    student_profile.get("course") if student_profile else "",
                )
            ...
            # replace with advisor siape
                ...
                raw_str = raw_str.replace(
                    parser_substr,
                    advisor_profile.get("siape") if advisor_profile else "",
                )
            ...
            # conditional parsing
                ...
                # gender differences
                if (
                    "ifStudentMale?" in command
                    and student_data
                    and student_data.get("gender")
                ):
                    raw_str = raw_str.replace(
                        parser_substr,
                        command.replace("ifStudentMale?", "").split(":::")[
                            0 if student_data["gender"] == "M" else 1
                        ],
                    )
                elif (
                    "ifAdvisorMale?" in command
                    and advisor_data
                    and advisor_data.get("gender")
                ):
                    raw_str = raw_str.replace(
                        parser_substr,
                        command.replace("ifAdvisorMale?", "").split(":::")[
                ...
                # course differences, works only with students
                    ...
                    "ifBCCStudent?" in command
                    and student_profile
                    and student_profile.get("course")
                ):
                    raw_str = raw_str.replace(
                        parser_substr,
                        command.replace("ifBCCStudent?", "").split(":::")[
