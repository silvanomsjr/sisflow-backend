...
CREATE TABLE etapa_solicitacao(
    ...
    nome_pagina_estatica VARCHAR(20),
    PRIMARY KEY (id),
    FOREIGN KEY (id_solicitacao) REFERENCES solicitacao(id)
);

CREATE TABLE etapa_solicitacao_pagina_dinamica(
	id_etapa_solicitacao INT NOT NULL,
    titulo VARCHAR(100),
    perfis_permitidos VARCHAR(50),
    top_inner_html VARCHAR(500),
    mid_inner_html VARCHAR(500),
    bot_inner_html VARCHAR(500),
    anexos_solicitados JSON,
